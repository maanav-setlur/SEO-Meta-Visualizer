...
import { type SeoIssue, type SeoIssueLevel } from "@shared/schema";
...
export function IssueList({ issues }: IssueListProps) {
  // Sort issues by severity: critical > warning > info > success
    ...
    <div className="space-y-3">
        ...
        return (
          ...
          >
            ...
            <div className="flex-1">
              ...
              {issue.field && (
                <p className="text-xs mt-1 opacity-70 font-mono">Field: {issue.field}</p>
              )}
            </div>
          </div>
        );
      ...
      {sortedIssues.length === 0 && (
        <div className="text-center py-12 text-slate-500 bg-slate-50 rounded-xl border border-dashed border-slate-200">
          ...
          <p className="text-sm">This page looks perfectly optimized.</p>
        </div>
      )}
    </div>
  );
